...
void Audio::playMusic(std::string nameMusic, bool isMidi) {
    ...
    if (music == NULL) {
        ...
        msgError << "Error al cargar la musica " << path.str().c_str() << ", SDL_mixer Error: " << Mix_GetError();
...
void Audio::playSound(std::string nameSound) {
    ...
    if (sound == NULL) {
        std::stringstream msgError;
        msgError << "Error al cargar el sonido " << path.str().c_str() << ", SDL_mixer Error: " << Mix_GetError();
        Error::logError(msgError.str().c_str());
    } else {
