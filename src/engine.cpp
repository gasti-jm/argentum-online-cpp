...
bool Engine::init() {
	...
	if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0) {
        msgError << "SDL could not initialize! SDL_Error: " << SDL_GetError();
	...
	if (!eWindow->init()) {
        msgError << "Window could not be created! SDL_Error: " << SDL_GetError();
        Error::logError(msgError.str().c_str());
		return false;
	}
	...
	if (renderer == NULL) {
        msgError << "Render could not be created! SDL_Error: " << SDL_GetError();
        Error::logError(msgError.str().c_str());
		return false;
	}
	...
	if (!(IMG_Init(imgFlags) & imgFlags)) {
        msgError << "SDL_Image could not initialize! SDL_image Error: " << IMG_GetError();
        Error::logError(msgError.str().c_str());
		return false;
	}
    ...
    User.x = User.y = 50;
    mainGUI = new GUI(renderer, "mainInterface.png");
    userInventory = new Inventory(renderer, 25);
...
void Engine::close() {
    if (mainGUI != NULL) delete mainGUI;
    if (userInventory != NULL) delete userInventory;
...
void Engine::showNextFrame() 
{
    ...
    if (inChangeRenderer) {
        ...
        mainGUI->setNewRenderer(renderer);
        userInventory->setNewRenderer(renderer);
    ...
    if (UserMoving){
        if (AddToUserPos.X != 0){
            OffsetCounterX = OffsetCounterX - SCROLL_PIXELS_PER_FRAME * AddToUserPos.X * timerTicksPerFrame;
            if (abs(OffsetCounterX) >= abs(TILE_PIXEL_SIZE * AddToUserPos.X)){
                OffsetCounterX = AddToUserPos.X = 0.0f;
                UserMoving = false;
            }
        }

        if (AddToUserPos.Y != 0){
            OffsetCounterY = OffsetCounterY - SCROLL_PIXELS_PER_FRAME * AddToUserPos.Y * timerTicksPerFrame;
            if (abs(OffsetCounterY) >= abs(TILE_PIXEL_SIZE * AddToUserPos.Y)){
                OffsetCounterY = AddToUserPos.Y = 0.0f;
                UserMoving = false;
            }
        }
    }
    ...
    int guiPosX = 296;
    if (resources.getCFG().screen_width == 800) guiPosX = 380;
    mainGUI->renderGUI(resources.getCFG().screen_width / 2 - guiPosX, resources.getCFG().screen_height - 90);
    userInventory->drawInterface(resources.getCFG().screen_width - 169, resources.getCFG().screen_height - 186);
    renderInventory();
...
void Engine::renderInventory() {
    for (int i = 0; i < userInventory->getMaxSlots(); i++) {
        userInventory->drawSlot(resources.getCFG().screen_width - 160,
                                resources.getCFG().screen_height - 175);
    }
}
    ...
    if (SDL_PollEvent(&e) != 0) 
    {
        ...
        else if (e.type == SDL_KEYDOWN) 
        {
			switch (e.key.keysym.sym) 
            {
                ...
                case SDLK_n: viewNicks = !viewNicks; break;
    ...
    for (int a = 0; a < static_cast<int>(text.length()); a++) {
        ...
        if (ascii != 32) { // espacio
            if (resources.getFont(nType).ascii_code[ascii] > 0) {
                ...
                drawTextureGrhIndex(resources.getFont(nType).ascii_code[ascii] + 100, (x + d) + 2, y + 1, r, g, b);

                drawTextureGrhIndex(resources.getFont(nType).ascii_code[ascii], (x + d) + 1, y, r, g, b);
                d = d + resources.getGrhData(resources.getFont(nType).ascii_code[ascii]).pixelWidth;
            }
        } else {
            d = d + 4;
        }
